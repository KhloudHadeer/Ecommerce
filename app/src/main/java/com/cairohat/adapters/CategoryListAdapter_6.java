...
import static com.cairohat.app.App.getContext;
...
public class CategoryListAdapter_6 extends RecyclerView.Adapter<CategoryListAdapter_6.MyViewHolder> {
    ...
    public CategoryListAdapter_6(Context context, List<CategoryData> categoriesList, boolean isSubCategory) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        final CategoryData categoryDetails = categoriesList.get(position);

        holder.category_title.setText(categoryDetails.getName());
        holder.category_products.setText("Total product"/*categoryDetails.getTotalProducts() + " "+ context.getString(R.string.products)*/);


        List<CategoryData> subCategoriesList = new ArrayList<>();

        for (int i=0;  i<allCategoriesList.size();  i++) {
            // Get Subcategories from all Categories
            if (allCategoriesList.get(i).getParent().equalsIgnoreCase(categoryDetails.getId())) {
